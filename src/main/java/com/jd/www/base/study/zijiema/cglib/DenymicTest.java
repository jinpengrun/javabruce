...
import com.jd.www.base.study.classloader.ClassMain;
import javassist.*;
 ...
 * 我们动态实现一个代理类  利用 javassist 来实现
 *
 * 这样也不可取创建过程当中包含了太多的业务代码，而且封装也不够多
 */
public class DenymicTest {


    public static void main(String[]args)throws  Exception{
       Class c = new ClassMain().defineClassFromClassFile("com.jd.www.base.study.zijiema.cglib.StationProxyDenymic",createProxy());
    }

    private static byte[] createProxy() throws Exception
    {
        ClassPool pool = ClassPool.getDefault();

        CtClass cc = pool.makeClass("com.jd.www.base.study.zijiema.cglib.StationProxyDenymic");

        //设置接口
        CtClass interface1 = pool.get("com.jd.www.base.study.zijiema.cglib.TicketService");
        cc.setInterfaces(new CtClass[]{interface1});

        //设置Field
        CtField field = CtField.make("private com.jd.www.base.study.zijiema.cglib.Station station;", cc);

        cc.addField(field);

        CtClass stationClass = pool.get("com.jd.www.base.study.zijiema.cglib.Station");
        ...
        //添加widthraw方法
        CtMethod changeTicket = CtMethod.make("public void changeTicket() {}", cc);
        changeTicket.setBody("{this.showAlertInfo(\"××××欢迎光临本代售点，改签没有费用多谢！××××\");"
                        + "station.withdraw();"
                        + "this.takeHandlingFee();}"
        );
        cc.addMethod(changeTicket);
